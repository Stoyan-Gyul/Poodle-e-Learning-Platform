...
from data.common.responses import InternalServerError_500, NotFound_404, Forbidden_403, BadRequest_400, Conflict_409, OK_200
...
def view_enrolled_courses(title: str | None = None,
    ...
    if user.is_student():
        ...
        return Conflict_409('Only students can view their enrolled courses!')
...
def course_rating(course_id: int, rating: int=Body(embed=True, ge=0, le=10), authorization: str =Header(None)):
    ...
    if user.is_student():
        ...
        if result:
            return OK_200('Student successfully rated this course!')
    
    return Conflict_409('You are not allowed to rate this course!')
...
def get_reports_by_course_id(course_id: int, authorization: str = Header()):
    ...
    if not user.is_course_owner(course):
        return Forbidden_403('Only the course owner has access to its reports.')
...
def create_course(course: Course, authorization: str = Header(None)):
    ...
    if not user.is_teacher():
        return Forbidden_403('Only a teacher can create courses.')

    if course.tag_ids == []:
        return BadRequest_400('Must contain at least one tag')
    if course.objective_ids == []:
        return BadRequest_400('Must contain at least one objective')

    tags = courses_service.get_tags(course.tag_ids)
    if len(tags) < len(course.tag_ids):
        return BadRequest_400('Must contain existing tags')
    objectives = courses_service.get_objectives(course.objective_ids)
    if len(objectives) < len(course.objective_ids):
        return BadRequest_400('Must contain existing objectives')
...
def update_course(course_id: int, data: CourseUpdate, authorization: str = Header()):
    ...
    if not user.is_course_owner(course):
        return Forbidden_403('Only the course owner can modify the course.')

    updated_course = courses_service.update_course(data, course)
    if updated_course is None:
        return InternalServerError_500('Failed to update the course.')
...
def deactivate_course(course_id: int, authorization: str = Header(None)):
    ...
    if not user.is_course_owner(course):
        return Forbidden_403('Only the course owner can deactivate the course.')
...
def upload_pic_to_course(course_id: int, pic: UploadFile, authorization: str = Header(None),):
    ...
    return OK_200("Picture uploaded successfully")
...
def create_section(course_id: int, section: Section, authorization: str = Header()):
    ...
    if not user.is_course_owner(course):
        return Forbidden_403('Only the course owner can create sections within it.')
...
def update_section(course_id: int, section_id: int, section: Section, authorization: str = Header()):
    ...
    if not user.is_course_owner(course):
        return Forbidden_403('Only the course owner can update sections within it.')

    existing_section = courses_service.get_section_by_id(section_id)
    if existing_section is None:
        return NotFound_404(f'Section {section_id} does not exist!')
    else:
...
def view_section(course_id: int,section_id: int, authorization: str = Header()):
    ...
    if not courses_service.is_student_enrolled_in_course(course_id, user_id):
        return NotFound_404('This student is not enrolled in this course.')
    # verify if course has section
    if not courses_service.has_course_section(course_id, section_id):
        return NotFound_404('This course has not this section')
...
def admin_removes_student_from_course(course_id: int, student_id: int, authorization: str = Header()):
    ...
    if not courses_service.is_student_enrolled_in_course(course_id,student_id):
        return Conflict_409(f'The student with ID:{student_id} is not enrolled in course with ID:{course_id}.')
    
    user = get_user_or_raise_401(authorization)
    if user.is_admin():
        if courses_service.admin_removes_student_from_course(course_id,student_id):
            return OK_200(f'The student with ID:{student_id} is removed from course with ID:{course_id}.')
        return Conflict_409('Something went wrong.Try again.')

    return Conflict_409('You are not administator.')
...
def admin_views_students_ratings(course_id: int, authorization: str = Header()):
        ...
        if user.is_admin():
            ...
            if history:
                return history
            return NotFound_404(f'There is no students in course {course_id}')
        ...
        if courses_service.is_course_active(course_id):
            ...
            if user.is_admin():
                if courses_service.admin_removes_course(course_id):
                    return OK_200(f'Course {course_id} has been hidden.')
                return InternalServerError_500('Something went wrong. The course has been hiden but notifications were not sent.')
        return Conflict_409(f'Course {course_id} is already hidden!')
